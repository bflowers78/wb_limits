...
class Seance:
    ...
    def send_captcha_screenshot(self):
        ...
        captcha_answer = bot.send_photo(MY_ID, open('captcha_screenshot.png', "rb"))
    ...
    def get_sms_code(self):
        sms_code = bot.send_message(MY_ID, 'Captcha введена верно, отправте код из СМС')
    ...
    def send_sms_code(self, sms_code):
        ...
        if self.driver.current_url != params['url']:  # работает не корректно
            ...
            bot.send_message(MY_ID, 'Успешный вход')
            self.parsing()
...
def check_changes(data_new):
    ...
    data_old = get_data()
    if not data_new:
        bot.send_message(MY_ID, 'Бот вернул пустой словарь')
        return
            ...
            for warehouse in data_new[date].keys():
                    for cargo in data_new[date][warehouse].keys():
                        try:
                            ...
                            bot.send_message(MY_ID, f'Ключ {e.args[0]} отсутствует в старом словаре')
                            time.sleep(1)
        save_data(data_new)
        return changes
...
def rotor_changes(changes):
    ...
    for date, wh, cargo, value in changes:
        if wh in wh_interest and cargo == 'Короба' and value == 'Бесплатно':
            bot.send_message(MY_ID, f'Произошли изменения: {date} / {wh} / {value}')
            time.sleep(2)
